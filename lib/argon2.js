(function (root, factory) {
    ...
    function setRuntime(simd, threads) {
        if (loadModule._module) {
            unloadRuntime();
        }

        if (threads) {
            global.loadArgon2WasmModule = async function () {
                // eslint-disable-next-line @typescript-eslint/no-var-requires
                return Promise.resolve(require('../dist/argon2-threads.js'));
            }
        }

        if (simd || threads) {
            global.loadArgon2WasmBinary = async function () {
                return Promise.resolve(require(`../dist/argon2${simd? "-simd": ""}${threads? "-threads": ""}.wasm`)).then(
                    (wasmModule) => {
                        return decodeWasmBinary(wasmModule);
                    }
                );
    ...
    async function loadOptimizedRuntime() {
        var featureDetect = require("wasm-feature-detect");
        var simd = await featureDetect.simd();
        var threads = await featureDetect.threads();

        // iOS seems broken argon2 and threads enabled
        var isiOS = navigator.userAgent.indexOf('AppleWebKit') != -1 && ('ontouchstart' in window || navigator.maxTouchPoints);

        setRuntime(simd, threads && !isiOS);
    }
